...
import { AfterContentChecked, AfterViewChecked, AfterViewInit, Component, DoCheck, ElementRef, NgZone, OnChanges, OnInit, ViewChild } from '@angular/core';
...
export class FootkeyChartComponent implements OnInit, AfterViewInit,AfterContentChecked,AfterViewChecked{
  ...
  public getFootkeyChart(){
    ...
    if(!errorFlg){
      this.mxService.getFootkeyChart(this.year, this.systemId)
        .subscribe(data => { 
  ...
  public setFootkey(){
    ...
    this.context!.moveTo(this.footkeyChart.footkeyList[0].xPoint,this.footkeyChart.footkeyList[0].yPoint);
  ...
  public setResult(){
    ...
    this.context!.beginPath();
    this.context!.moveTo(this.footkeyChart.resultList[0].xPoint,this.footkeyChart.resultList[0].yPoint)
